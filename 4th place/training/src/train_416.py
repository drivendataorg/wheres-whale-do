...
MODEL_DIR = f'{MODEL_DIR}/models_eff5_r{RANDOM_STATE}_{IMG_SIZE}'
...
class CustLoss(nn.Module):
    def __init__(self,cfg):
        ...
        # self.lfn = ArcFaceLoss(s=cfg.s,m=cfg.m)
        
        # self.lfn = losses.ArcFaceLoss(num_classes=NUM_CLASSES,embedding_size=cfg.embedding_size,margin=0.1,scale=0.16)
...
# model = BelugaNet(arch, in_feats=in_feats,emb_size=emb_size,cut=cut)
...
learn = Learner(dls, model, splitter=model_split,    
                ...
                cbs=[#CustLoader,
                    ...
                    ],metrics=[#accuracy,map20,#
                               maplb
                              ],
               )
learn.to_fp16()
with learn.no_bar():
    learn.fit_one_cycle(10, lr_max=[1e-4,5e-4,1e-3,1e-4]); torch.save(learn.model.state_dict(), f'{MODEL_DIR}/{cfg.backbone}_f{FOLD}_p1.pth')
