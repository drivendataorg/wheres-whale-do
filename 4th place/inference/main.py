...
models_eff7_r41_448_v4 = [f'models/models_eff7_r41_448_v4/tf_efficientnet_b7_ns_f{i}_p1.pth' for i in range(0,5)]
...
def main():
    ...
    dataloader = DataLoader(dataset, batch_size=11,num_workers=NUM_WORKERS)
    ...
    for path in models_eff7_r41_448_v4:
        weights = torch.load(path,map_location=DEVICE)
        model.load_state_dict(weights,strict=False)
        model.to(DEVICE); model.eval()
        logger.info(f"Precomputing embeddings for model {path}")
    ...
    dataloader = DataLoader(dataset, batch_size=6,num_workers=NUM_WORKERS)
