...
#         embeddings = embeddings_2017
    ...
    for qry in qry_df.itertuples():
        ...
        sims[sims < 0 ] = 0
        sims[sims > 1 ] = 1
        top4 = pd.Series(sims, index=_db_embeddings.index).sort_values(0, ascending=False).head(4)
        
        if only_pseudo:
            qry2_embedding = emb2.loc[[top4.index[0]]]
            sims = cosine_similarity(qry2_embedding, _db_embeddings2)[0]
            sims[sims < 0 ] = 0
            sims[sims > 1 ] = 1
            top4 = pd.Series(sims, index=_db_embeddings2.index).sort_values(0, ascending=False).head(4)
            qry3_embedding = emb2.loc[[top4.index[1]]]
            qry4_embedding = emb2.loc[[top4.index[2]]]
            qry5_embedding = emb2.loc[[top4.index[3]]]
            sims2 = cosine_similarity(qry2_embedding, _db_embeddings2)[0]
            sims3 = cosine_similarity(qry3_embedding, _db_embeddings2)[0]
            sims4 = cosine_similarity(qry4_embedding, _db_embeddings2)[0]
            sims5 = cosine_similarity(qry5_embedding, _db_embeddings2)[0]
        elif skip:
            ...
            sims = cosine_similarity(qry2_embedding, qry_embeddings)[0]
            sims[sims < 0 ] = 0
            sims[sims > 1 ] = 1
            top4 = pd.Series(sims, index=qry_embeddings.index).sort_values(0, ascending=False).head(4)
            qry2_embedding = embeddings.loc[[qry.query_image_id]]
            qry3_embedding = embeddings.loc[[top4.index[1]]]
            qry4_embedding = embeddings.loc[[top4.index[2]]]
            qry5_embedding = embeddings.loc[[top4.index[3]]]
            sims2 = cosine_similarity(qry2_embedding, _db_embeddings)[0]
            sims3 = cosine_similarity(qry3_embedding, _db_embeddings)[0]
            sims4 = cosine_similarity(qry4_embedding, _db_embeddings)[0]
            sims5 = cosine_similarity(qry5_embedding, _db_embeddings)[0]
            
        else:
            qry2_embedding = embeddings.loc[[top4.index[0]]]
            qry3_embedding = embeddings.loc[[top4.index[1]]]
            qry4_embedding = embeddings.loc[[top4.index[2]]]
            qry5_embedding = embeddings.loc[[top4.index[3]]]
            sims2 = cosine_similarity(qry2_embedding, _db_embeddings)[0]
        ...
        sims5[sims5 > 1 ] = 1
        if only_pseudo:
            sims = 0.4*sims2 + 0.3*sims3 + 0.2*sims4 + 0.1*sims5
        elif skip:
            sims = 0.3*sims2 + 0.3*sims3 + 0.2*sims4 + 0.2*sims5
        else:
            sims = (sims + 0.4*sims2 + 0.3*sims3 + 0.2*sims4 + 0.1*sims5) / 2        
