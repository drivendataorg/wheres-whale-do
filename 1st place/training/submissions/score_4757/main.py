...
#     batch_embeddings5 = batch_embeddings5.detach().cpu().numpy()
    ...
    batch_embeddings1_literal = (model1_literal(batch["image"].to(CFG.device)).detach().cpu().numpy() + model1_literal(batch["image2"].to(CFG.device)).detach().cpu().numpy()) / 2
    batch_embeddings2_literal = (model2_literal(batch["image"].to(CFG.device)).detach().cpu().numpy() + model2_literal(batch["image2"].to(CFG.device)).detach().cpu().numpy()) / 2
