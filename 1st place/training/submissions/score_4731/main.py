...
for batch in tqdm(dataloader, total=len(dataloader),disable=True):
    batch_embeddings6 = model6(batch["image"].to(CFG.device))
    batch_embeddings = model6(batch["image2"].to(CFG.device))
    batch_embeddings6 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings6.detach().cpu().numpy()) / 2
    batch_embeddings7 = model7(batch["image"].to(CFG.device))
    batch_embeddings = model7(batch["image2"].to(CFG.device))
    batch_embeddings7 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings7.detach().cpu().numpy()) / 2
    batch_embeddings8 = model8(batch["image"].to(CFG.device))
    batch_embeddings = model8(batch["image2"].to(CFG.device))
    batch_embeddings8 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings8.detach().cpu().numpy()) / 2
...
state1_literal = torch.load("model_tf_efficientnet_b5_ns_IMG_SIZE_512_arcface_literal_f2_3-09.bin",map_location=torch.device(CFG.device))
...
state2_literal = torch.load("model_tf_efficientnet_b5_ns_IMG_SIZE_512_arcface_literal_tail_f2_3-81.bin",map_location=torch.device(CFG.device))
...
for batch in tqdm(dataloader, total=len(dataloader),disable=True):
    ...
    batch_embeddings = model1(batch["image2"].to(CFG.device))
    batch_embeddings1 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings1.detach().cpu().numpy()) / 2
    batch_embeddings2 = model2(batch["image"].to(CFG.device))
    batch_embeddings = model2(batch["image2"].to(CFG.device))
    batch_embeddings2 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings2.detach().cpu().numpy()) / 2
    batch_embeddings3 = model3(batch["image"].to(CFG.device))
    batch_embeddings = model3(batch["image2"].to(CFG.device))
    batch_embeddings3 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings3.detach().cpu().numpy()) / 2
    batch_embeddings4 = model4(batch["image"].to(CFG.device))
    batch_embeddings = model4(batch["image2"].to(CFG.device))
    batch_embeddings4 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings4.detach().cpu().numpy()) / 2
    batch_embeddings5 = model5(batch["image"].to(CFG.device))
    batch_embeddings = model5(batch["image2"].to(CFG.device))
    batch_embeddings5 = (batch_embeddings.detach().cpu().numpy() + batch_embeddings5.detach().cpu().numpy()) / 2
...
#     batch_embeddings5 = batch_embeddings5.detach().cpu().numpy()
    ...
    batch_embeddings1_literal = model1_literal(batch["image"].to(CFG.device))
    batch_embeddings2_literal = model2_literal(batch["image"].to(CFG.device))
    ...
    batch_embeddings1_differentyears = model1_differentyears(batch["image"].to(CFG.device))
    batch_embeddings2_differentyears = model2_differentyears(batch["image"].to(CFG.device))
...
#     batch_embeddings_sameyear = np.concatenate((batch_embeddings1_sameyear, batch_embeddings2_sameyear, batch_embeddings3_sameyear, batch_embeddings4_sameyear, batch_embeddings5_sameyear), 1)
    batch_embeddings_literal = np.concatenate((batch_embeddings1_literal, batch_embeddings2_literal), 1)
...
del model1, model2, model3, model4, model5, model1_literal, model2_literal ,model1_differentyears, model2_differentyears  
gc.collect()
torch.cuda.empty_cache()
...
#         embeddings = embeddings_2017
    ...
    for qry in qry_df.itertuples():
        ...
        if only_pseudo:
            sims = 0.5*sims + 0.5*sims2
        elif skip:
            sims = 0.5*sims + 0.5*sims2
        else:
            sims = 0.4 * sims +  0.4 * sims2 + 0.2 * sims3 
