...
class WhaleDataset_testing(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {"image_id": self.csv.index[index], "image": image3, "image2": image4, "image3": image3, "image4": image4}
...
#         embeddings_eff2 = pd.concat([embeddings_differentyears, embeddings_eff2], 1)
    ...
    for qry in qry_df.itertuples():
        ...
        if only_pseudo:
            ...
            sims2 = 0.625 * sims2 + 0.375 * sims3
            sims2[sims2 < 0 ] = 0
            sims2[sims2 > 1 ] = 1
            top2 = pd.Series(sims2, index=qry_embeddings.index).sort_values(0, ascending=False).head(3)
            qry2_embedding = embeddings.loc[[top2.index[1]]]            
            sims2 = cosine_similarity(qry2_embedding, _db_embeddings)[0]
            qry3_embedding = embeddings.loc[[top2.index[2]]]            
